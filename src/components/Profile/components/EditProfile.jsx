...
export default function EditProfile(){
    ...
    useEffect(() => {
        return async function fetchdata() {
            await axios.get(`${BASE_URL}/api/auth/me/${token.id}
            `).then(res => {
    ...
    try {
        let res = await axios.patch(`${BASE_URL}/api/auth/me/${token.id}`, saveData)
        if (res.data.success) {
            Swal.fire({
                icon: 'success',
        ...
        return (
            ...
            <div className="header-columns-edit">
                ...
                <div className="col-3-header">
                    <button className="btn-edit-profile" form="myform" onClick={e=> saveData(e)}type="">Submit Changes</button>
                </div>
            </div>
